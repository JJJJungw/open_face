...
REVIEW = {
    ...
    "decode-short":
        ...
        "주세요.",
    "bitrate-out-of-band":
        "결과물 비트레이트가 납품 대역을 벗어났습니다. 그대로 내면 검수에서 "
        "반려될 수 있어 확인이 필요합니다.",
}
...
NOTICE = {
    ...
    "bitrate-unverified": "결과물 비트레이트를 재지 못해 대역 검사를 건너뛰었습니다.",
    # 사람을 부를 일이 아니다 — 원본이 이미 대역 아래였고, 올려 담지 않은 것이
    # 의도한 동작이다. 다만 어느 파일이 그랬는지는 남는다.
    "source-below-band": "원본이 납품 대역보다 낮아 그 수준으로 내보냈습니다.",
...
STAGE_FACE = {
    ...
    # 이 줄이 없어서 GPU 메모리 부족이 저쪽 화면에 "알 수 없는 오류" 로 나갔다.
    # 같은 상황을 API 경로는 정확히 말한다(errors.GPU_OUT_OF_MEMORY) — 014 가
    # 판정을 통일했는데 **문구 표만 안 따라온** 것이다.
    "oom": ("GPU 메모리가 부족합니다",
            "batch_size 나 imgsz 를 낮추면 통과할 수 있습니다"),
