...
# 검수 대기 건의 보관 기간. **0 = 안 지운다** (기본값).
#
# 일반 TTL(2시간)을 그대로 태우면, 밤에 300건을 돌려 놓고 아침에 오면 검수
# 목록이 비어 있다. 지워진 것은 "사람이 봐야 한다" 는 사실 그 자체이고,
# 결과물은 버킷에 그대로 남아 납품 폴더에 섞인다 — 이 기능을 만든 목적이
# 통째로 무효가 되는 자리다. 판정 전에는 시간으로 지우지 않는다.
REVIEW_TTL = int(os.environ.get("FA_REVIEW_TTL_MIN", 0)) * 60
