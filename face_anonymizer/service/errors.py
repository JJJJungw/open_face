...
# ── 붙지 않는 이유는 다섯 가지고, 사람이 할 일이 전부 다르다 ────────────────
#
# 예전에는 넷이 하나로 뭉쳐 있었다. 키가 아예 없어도 "리전 설정과 네트워크
# 상태를 확인해 주세요" 가 나갔다 — 리전도 네트워크도 멀쩡한데 그 둘을 뒤지게
# 만드는 문구다. 게다가 그게 ``retryable=True`` 라서, 워커는 절대 성공할 수
# 없는 호출을 95초짜리 창 안에서 네 번 반복했다.
#
#   키가 없다        → 넣거나 환경에 둔다        (재시도해도 같다)
#   키가 틀렸다      → 키를 다시 본다            (재시도해도 같다)
#   권한이 없다      → 콘솔에서 정책을 고친다    (재시도해도 같다)
#   버킷이 없다      → 이름·리전을 다시 본다     (재시도해도 같다)
#   못 닿는다        → 주소·망을 본다            (**이것만 재시도 대상이다**)
S3_NO_CREDENTIALS = _p(
    "s3_no_credentials", 502, "자격 증명을 찾지 못했습니다",
    "액세스 키가 어디에도 없습니다. 화면에서 넣으시거나, 환경 변수"
    "(AWS_ACCESS_KEY_ID · AWS_SECRET_ACCESS_KEY)나 ~/.aws/credentials 에 "
    "두시면 됩니다. AWS 라면 이 서버에 IAM 역할을 붙여도 됩니다.")
S3_BAD_CREDENTIALS = _p(
    "s3_bad_credentials", 502, "자격 증명이 올바르지 않습니다",
    "키는 있는데 저장소가 거절했습니다. 액세스 키와 시크릿 키를 다시 확인해 "
    "주세요. 다른 클라우드에서 발급한 키를 쓰고 있지는 않은지도 함께 보시면 "
    "됩니다.")
S3_ACCESS_DENIED = _p(
    "s3_access_denied", 502, "이 버킷에 대한 권한이 없습니다",
    "자격 증명은 확인됐지만 저장소가 이 동작을 거부했습니다. "
    "s3:ListBucket · s3:GetObject · s3:PutObject 권한을 확인해 주세요.")
S3_BUCKET_NOT_FOUND = _p(
    "s3_bucket_not_found", 502, "버킷을 찾지 못했습니다",
    "버킷 이름과 리전·엔드포인트가 서로 맞는지 확인해 주세요. "
    "이름은 맞는데 다른 리전에 있어도 이렇게 나옵니다.")
S3_UPSTREAM = _p(
    "s3_upstream", 502, "저장소에 닿지 못했습니다",
    "엔드포인트 주소와 네트워크 상태를 확인해 주세요.", retryable=True)
