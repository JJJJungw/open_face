...
**규칙은 하나다 — 올리지 않고, 목표에서 자른다.**

    목표 = min(원본 비트레이트, 3200k)

원본이 대역 위면 3200k 로 내려온다(정상적인 다운코딩). 원본이 아래면 **그
수준으로 나간다.** 올려 담는 것은 정보를 만들어 내지 못한다 — 압축 열화를
고화질로 보존할 뿐이고 용량만 는다. 실측(720p 방송 클립, 원본 312 kbps):

| 방식 | 비트레이트 | SSIM | PSNR |
|---|---|---|---|
| 원본 그대로 | 315 kbps | 0.9937 | 47.4 dB |
| 대역 강제 | 3203 kbps | 0.9984 | 56.7 dB |

10배를 써서 사는 것이 **사람 눈 구분 한계 위쪽**의 차이다. 900건이면 6 GB 와
60 GB 가 갈린다.

정한 비트레이트는 **CBR 로 못 박는다.** `-b:v` 는 목표 평균이지 하한이 아니라서,
정지에 가까운 컷은 인코더가 얼마든지 아낀다(실측 14 kbps). 스터핑을 켜야 선다.

결과물은 다시 잰다. 위끝을 넘으면 검수로 넘기고, 아래끝에 못 미치는 것은 **원본이
이미 그랬으면 기록만** 남긴다([issues/018](../docs/issues/018-the-delivery-band-was-not-enforced.md)).
